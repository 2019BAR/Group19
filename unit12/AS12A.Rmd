...
c(logit=logit, odd=odd, prob=prob)
```
<br> Ans: odd=0.12430 prob=0.11056
<br>
<br>
...
Blue: 綠點落在線上(y=1)的機率為0.5<br>
cyan: 從0.1~0.9的機率線，機率從左下到右上遞增。<br>

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>
看點落在哪兩條線的區間就可以估計y=1的機率<br>
當點落在藍線以上的區塊，代表其為綠點(y=1)的機率>0.5<br>
當點落在藍線以下的區塊，代表其為綠點(y=1)的機率<0.5<br>
<br><hr>
...
<span style="font-size:24px"> `r "\U1F5FF"` : </span>
What are the defintion of `logit` & `logistic function`? What is the relationship between them?<br>
logit：將odd取log函數，值會介於負無限大~正無限大，也就是x的線性組合。<br>
logistic function:將logit代入Sigmoid Function，其值會介於0到1之間。可求出類別變數的目標函數機率。<br>
當logit=0時，y=1的機率是0.5；<br>
logit>0: y=1的機率>0.5；<br>
logit<0: y=1的機率<0.5。<br>
<br><br><br><hr>
