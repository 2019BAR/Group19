...
# 2
#用State去找出CPS$MetroAreaCode中沒有出生地在非市中心的人
#Alaska,Wyoming
...
```{r}
#3
#用State去找出CPS$MetroAreaCode中沒有出生地在市中心的人
...
abs(0.3 - tapply(is.na(CPS$MetroAreaCode), CPS$State, mean)) %>% 
  sort %>% head
#要找出離0.3最近的數值，所以找出用0.3扣掉以後值的最小的6項
#Wisconsin
```
...
tapply(is.na(CPS$MetroAreaCode), CPS$State, mean) %>% 
  sort %>% tail
#用Region去分MetroAreaCode，叫出平均值，並叫出最後6項
#不要全部都是的，所以把「1」的扣掉不看
#Montana
```
...
#Laredo, TX
# 看每個MetroArea裡面，是Hispanic，的比例，並叫出最後六項
```
...
# 看每個MetroArea裡面，Race是Asian的比例，並叫出最後六項
#4
```
...
tapply(CPS$Education == "No high school diploma", CPS$MetroArea, mean, na.rm=T) %>% 
  sort %>% head 
# Iowa City, IA
# 看每個MetroArea裡面，沒有high school diploma"的比例，並叫出前六項
```
...
# Country
#用merge，搭配"CountryOfBirthCode"和"Code"去配對，創造一個新欄位叫Country
```
...
tapply(CPS$Country == 'India', CPS$MetroArea, sum, na.rm=T) %>% 
  sort %>% tail 
#找出最多印度出生的interviewees的metropolitan地區
...
#找出最多Brazil出生的interviewees的metropolitan地區
...
#找出最多Somalia出生的interviewees的metropolitan地區
