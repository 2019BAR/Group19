---
title: "AS3-2: 股市動態"
author: "第 19 組"
date: "`r Sys.time()`"
output:
  html_document:
    highlight: pygments
    theme: flatly
    css: style.css
---

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE}
# 這些程式碼設定網頁的格式，並安裝、載入一些基本的套件，請大家不要去改動<br>
rm(list=ls(all=T))
knitr::opts_chunk$set(comment = NA)
knitr::opts_knit$set(global.par = TRUE)
par(cex=0.8)
options(scipen=20, digits=4, width=90)
#
if(!require(pacman)) install.packages("pacman")
pacman::p_load(dplyr)
```
<hr>

**股票動態**

股票市場是買賣雙方交易公司股票的地方，也是個人和公司投資的最受歡迎的方式之一。現在估計世界股票市場規模達到數兆。紐約證券交易所位於紐約市，是世界上最大的股票市場。 紐約證券交易所約有2,800家上市公司。在這個問題上，我們將看看其中五家公司的每月股價：IB、通用電氣（GE）、寶潔、可口可樂和波音。此問題中使用的數據來自Infochimps。

使用`read.csv()`下載並讀取以下文件：

+ `data/IBMStock.csv`
+ `data/GEStock.csv`
+ `data/ProcterGambleStock.csv`
+ `data/CocaColaStock.csv`
+ `data/BoeingStock.csv` 

分別存入資料框`IBM`、`GE`、`ProcterGamble`、`CocaCola`和`Boeing`，
每個資料框都有兩個變量，描述如下：

+ `Date`：日期
+ `StockPrice`：給定月份公司的平均股價

在這個案例，我們將看看這些公司的股票動態如何隨著時間的推移而發生變化。

<br><hr>
```{r}
IBM = read.csv("data/IBMStock.csv")
GES = read.csv("data/GEStock.csv")
Pro = read.csv("data/ProcterGambleStock.csv")
Coca = read.csv("data/CocaColaStock.csv")
Boe = read.csv("data/BoeingStock.csv")
IBM$Date = as.Date(IBM$Date, "%m/%d/%y")
GES$Date = as.Date(GES$Date, "%m/%d/%y")
Pro$Date = as.Date(Pro$Date, "%m/%d/%y")
Coca$Date = as.Date(Coca$Date, "%m/%d/%y")
Boe$Date = as.Date(Boe$Date, "%m/%d/%y")
```

###  Section-1 Summary Statistics

__§ 1.1__ Our five datasets all have the same number of observations. How many observations are there in each data set?
```{r}
str(IBM) #struture of object IBM
#480
```


__§ 1.2__ What is the earliest year in our datasets?
```{r}
min(IBM$Date) #min of IBM date
```

__§ 1.3__ What is the latest year in our datasets?
```{r}
max(IBM$Date) #max of IBM date
```

__§ 1.4__ What is the mean stock price of IBM over this time period?
```{r}
mean(IBM$StockPrice) #mean of IBM's stockprice
```

__§ 1.5__ What is the minimum stock price of General Electric (GE) over this time period?
```{r}
min(GES$StockPrice) #min of GES's stockprice
```

__§ 1.6__ What is the maximum stock price of Coca-Cola over this time period?
```{r}
max(Coca$StockPrice) #min of Coca's stockprice
```

__§ 1.7__ What is the median stock price of Boeing over this time period?
```{r}
median(Boe$StockPrice) #median of Boe's stockprice
```

__§ 1.8__ What is the standard deviation of the stock price of Procter & Gamble over this time period?
```{r}
sd(Pro$StockPrice) # standard deviation of the stock price of Procter & Gamble 
```

<br><hr>

###  Section-2 Visualizing Stock Dynamics

__§ 2.1__ Around what year did Coca-Cola has its highest stock price in this time period? Around what year did Coca-Cola has its lowest stock price in this time period?
```{r}
plot(Coca$Date, Coca$StockPrice, type='l')  # 1973 1980
abline(v=Coca$Date[which.max(Coca$StockPrice)], col='green') #綠線最低點
abline(v=Coca$Date[which.min(Coca$StockPrice)], col='red') #紅線最高點
```

__§ 2.2__ In March of 2000, the technology bubble burst, and a stock market crash occurred. According to this plot, which company's stock dropped more?
```{r}
plot(Coca$Date, Coca$StockPrice, type='l', col='blue', lwd=1) #coca為藍色折線
lines(Pro$Date, Pro$StockPrice, col="green", lwd=1) #P&G為綠色折線
lines(IBM$Date, IBM$StockPrice, col="black", lwd=1) #IBM為黑色折線
lines(GES$Date, GES$StockPrice, col="purple", lwd=1) #GES為紫色折線
lines(Boe$Date, Boe$StockPrice, col="yellow", lwd=1) #Boe為黃色折線
abline(v = as.Date("2000-03-01"), lty=3, col='orange') #2000年的切線
abline(v = as.Date("1983-07-01"), lty=3, col='orange') #1983年的切線
#legend()為圖例
legend("topright",legend=c("Coke","P&G","IBM","GES","Boe"),col=c('blue','green','black','purple','yellow'),lwd=2)
#GES
```

__§ 2.3__ (a) Around 1983, the stock for one of these companies (Coca-Cola or Procter and Gamble) was going up, while the other was going down. Which one was going up? 
```{r}
#Coca-Cola
```

(b) In the time period shown in the plot, which stock generally has lower values?
```{r}
#Coca-Cola
```


<br><hr>

###  Section-3 Visualizing Stock Dynamics 1995-2005
```{r}
#[301]為1995年初 [432]為2005年底
#can use filiter() to find 1995-2005 location
```
```{r}
plot(Coca$Date[301:432], Coca$StockPrice[301:432], 
     type="l", col="red", ylim=c(0,210)) #coca為紅色折線
lines(Pro$Date[301:432],  Pro$StockPrice[301:432],  col="blue") # p&G為藍色折線
lines(IBM$Date[301:432], IBM$StockPrice[301:432], col="green") # IBM為綠色折線
lines(GES$Date[301:432],  GES$StockPrice[301:432],  col="purple") # GES為紫色折線
lines(Boe$Date[301:432],  Boe$StockPrice[301:432],  col="orange")  # Boe為橘色折線
abline(v = as.Date("2000-03-01"), lty=3, col='gray') # 2000年切線
abline(v = as.Date("1997-09-01"), lty=3, col='gray') # 1997年09月切線
abline(v = as.Date("1997-11-01"), lty=3, col='gray') # 1997年11月切線
#legend()為圖例
legend("topright",legend=c("Coke","P&G","IBM","GES","Boe"),col=c('red','blue','green','purple','orange'),lwd=1)
```
<br>__§ 3.1__ Which stock fell the most right after the technology bubble burst in March 2000?
```{r}
#IBM
```

__§ 3.2__ Which stock reaches the highest value in the time period 1995-2005?
```{r}
# IBM
```

__§ 3.3__ In October of 1997, there was a global stock market crash that was caused by an economic crisis in Asia. Comparing September 1997 to November 1997, which companies saw a decreasing trend in their stock price? (Select all that apply.)
```{r}
# P&G and Boeing
```

__§ 3.4__ In the last two years of this time period (2004 and 2005) which stock seems to be performing the best, in terms of increasing stock price?
```{r}
# Boeing
```


<br><hr>


###  Section-4 Monthly Trends

__§ 4.1__ For IBM, compare the monthly averages to the overall average stock price. In which months has IBM historically had a higher stock price (on average)? Select all that apply.
```{r}
tapply(IBM$StockPrice, format(IBM$Date,"%m"), mean) - mean(IBM$StockPrice)
#February,選取扣掉平均值最大者
```

__§ 4.2__ General Electric and Coca-Cola both have their highest average  price in the same month. Which month is this?
```{r}
stocklist= list(Boe=Boe,Coca=Coca,GES=GES,IBM=IBM,Pro=Pro) #stocklist為五個公司的股價，list(x=x,y=y....)將各公司股價連結成一個list
```
```{r}
sapply(stocklist, function(df) {tapply(df$Stock, format(df$Date,"%m"), mean) - mean(df$Stock)})
#April
#tapply(df$Stock, format(df$Date,"%m"), mean) - mean(df$Stock) 代表按照月份對stock進行分組後再去扣掉所有公司的平均股價
#sapply(x,fun{})代表對一個x的元素進行fun
```

__§ 4.3__ For the months of December and January, every company's average stock is higher in one month and lower in the other. In which month are the stock prices lower?
```{r}
#December
```

<br><br><br>

